...
<template>
    <div v-if="item" class="flex items-center space-x-2">
      <!-- Only show controls if it's not a box or if it's an empty box -->
      <template v-if="!item.isBox || (item.isBox && !hasBoxContents(item))">
        <!-- Present button -->
        <button 
      ...
      <!-- For boxes with contents, only show the note button -->
      <template v-else>
        <button 
          @click.stop="$emit('open-note', item)"
          class="p-1 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
  ...
  console.log('ItemControls props:', props); // Add this for debugging
